...
import { saveHistoryToLibrary, unsaveHistoryFromLibrary, getDesignBySlug } from '@/lib/api';
      ...
      try {
        ...
        if (response.success && response.design) {
          ...
          const mappedColors = (design.colors || []).map((c: string) => ({ 
            hex: c, 
            percentage: Math.round(100 / (design.colors?.length || 1)) 
          }));
          ...
          setError(t('common.notFound') || 'Design not found');
        }
      } catch (err) {
